[CmdletBinding()]
param (
    [string] $DeploymentDataLocation = 'japaneast',
...
if ($ValidateOnly) {
    $params = @{
        Location     = $DeploymentDataLocation
    ...
    $result = Test-AzDeployment @params
    ...
    $params = @{
        Name                    = ('{0}-{1}'-f (Get-Item -LiteralPath $templateFilePath).BaseName, (Get-Date).ToUniversalTime().ToString('yyyMMdd-HHmm'))
        Location                = $DeploymentDataLocation
        TemplateFile            = $templateFilePath
    ...
    try {
        ...
        $error[0]
        Get-AzDeploymentOperation -DeploymentName $params.Name
